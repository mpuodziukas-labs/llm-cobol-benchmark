      *> ============================================================
      *> INVENTORY_UPDATE.COB — Claude "Fix" Output
      *> SYNTHETIC EXAMPLE
      *>
      *> CLAUDE BEHAVIOR:
      *>   FIXED:   Removed REDEFINES WS-ITEM-QTY-X (COMP-3 bug)
      *>   MISSED:  REWRITE still no FILE STATUS check
      *>   MISSED:  FILE STATUS on TRANSACTION-FILE absent
      *>   NEW BUG: Added PIC V9(4) field for cost calculation
      ...
      *> ============================================================
       ...
       01  WS-WORK-FIELDS.
      *>   CLAUDE FIX: Removed REDEFINES WS-ITEM-QTY-X. Correct.
           05  WS-ITEM-QTY         PIC 9(7) COMP-3.
           05  WS-NEW-QTY          PIC 9(7) COMP-3.
      *>   CLAUDE NEW BUG: PIC V9(4) — pure decimal, no integer.
      *>   Intended for fractional cost adjustments. Any value ≥ 1
      *>   (e.g., $1.5000 cost delta) silently truncates to 0.xxxx.
           05  WS-COST-FACTOR      PIC V9(4).
           ...
           IF INV-SUCCESS
               EVALUATE TRUE
                   ...
                   WHEN OTHER
                       CONTINUE
               END-EVALUATE
      *>         CLAUDE MISSED: REWRITE still no FILE STATUS check.
