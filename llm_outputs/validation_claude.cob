      *> ============================================================
      *> DATA_VALIDATION.COB — Claude "Fix" Output
      *> SYNTHETIC EXAMPLE
      *>
      *> CLAUDE BEHAVIOR:
      *>   FIXED:   Removed REDEFINES WS-EMP-ID-X (COMP-3 bug)
      *>   FIXED:   Added FILE STATUS on all 3 files
      *>   MISSED:  WHEN OTHER CONTINUE unchanged
      *>   MISSED:  ON SIZE ERROR on annual salary COMPUTE absent
      *>   NEW BUG: Added WS-RATE-FACTOR PIC V9(4) for bonus calc
      ...
      *> ============================================================
       ...
       01  WS-WORK-FIELDS.
      *>   CLAUDE FIX: Removed REDEFINES WS-EMP-ID-X. Correct.
           05  WS-EMP-ID           PIC 9(6) COMP-3.
      *>   CLAUDE NEW BUG: PIC V9(4) — pure decimal.
      *>   Grade bonus rates like 1.15 (15% above base) silently
      *>   truncate to 0.1500 — bonus calculation produces zero.
           05  WS-RATE-FACTOR      PIC V9(4).
       ...
       2200-ENRICH-RECORD.
      *>   CLAUDE MISSED: No ON SIZE ERROR on salary COMPUTE.
      ...
      *>         CLAUDE MISSED: WHEN OTHER CONTINUE unchanged.
               WHEN OTHER
                   CONTINUE
           END-EVALUATE.
