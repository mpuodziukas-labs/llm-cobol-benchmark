      *> ============================================================
      *> DATA_VALIDATION.COB — Llama "Fix" Output
      *> SYNTHETIC EXAMPLE
      *>
      *> LLAMA BEHAVIOR:
      *>   FIXED:   Added FILE STATUS on INPUT-FILE only
      *>   MISSED:  WHEN OTHER CONTINUE unchanged
      *>   MISSED:  ON SIZE ERROR on salary COMPUTE absent
      *>   MISSED:  REDEFINES WS-EMP-ID-X still present
      *>   MISSED:  FILE STATUS on OUTPUT-FILE and ERROR-FILE absent
      *>   NEW BUG: Added COMPUTE for grade bonus without ON SIZE ERROR
      *> ============================================================
      ...
      *>   LLAMA NEW BUG: COMPUTE for grade bonus uses this field.
      *>   No ON SIZE ERROR — grade 99 × salary overflows silently.
           05  WS-GRADE-BONUS      PIC 9(7)V99.
      ...
      *>   LLAMA MISSED: REDEFINES on COMP-3 still present.
       ...
       2200-ENRICH-RECORD.
      *>   LLAMA MISSED: No ON SIZE ERROR on salary COMPUTE.
           COMPUTE WS-ANNUAL-SALARY =
               IN-MONTHLY-SALARY * 12
      *>   LLAMA NEW BUG: COMPUTE without ON SIZE ERROR.
      *>   Grade 99 at $99,999.99/mo → bonus overflows PIC 9(7)V99.
           COMPUTE WS-GRADE-BONUS =
               WS-ANNUAL-SALARY * IN-JOB-GRADE
